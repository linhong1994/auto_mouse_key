...
class OCR条件判断器类:
    ...
    def 识别并判断(self, 区域: tuple[int, int, int, int], 条件类型: str, 目标文本: str,
                    语言: str = "中文简体+英文", 超时时间: int = 10,
                    轮询间隔: int = 500, 超时处理: str = "跳过继续") -> bool:
        """一站式方法：识别屏幕区域文字并判断条件是否满足

        参数:
            区域: (左上X, 左上Y, 右下X, 右下Y)
            条件类型: 文本匹配/文本不匹配/文本包含/文字变化
            目标文本: 用于比较的目标文本
            语言: 识别语言
            超时时间: 超时秒数
            轮询间隔: 轮询毫秒
