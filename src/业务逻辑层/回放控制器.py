...
class 回放控制器类(QObject):
    ...
    def 启动回放(self, 步骤列表: list[操作步骤数据], 速度倍率: float = 1.0, 循环次数: int = 1) -> None:
        """启动脚本回放执行"""
    ...
    def _回放循环(self, 步骤列表: list[操作步骤数据], 速度倍率: float, 循环次数: int) -> None:
        """回放主循环"""
    ...
    def _执行OCR识别(self, 步骤: 操作步骤数据) -> bool:
        """执行OCR识别步骤"""
        if not self.OCR服务:
            return False
        try:
    ...
    def _执行OCR条件判断(self, 步骤: 操作步骤数据) -> bool:
        ...
        if not self.OCR条件判断器:
            return True
        from src.公共.数据结构 import OCR触发条件
        条件 = OCR触发条件(
            条件类型=步骤.OCR条件类型 or "文本匹配",
            识别区域=(步骤.OCR区域左上角X or 0, 步骤.OCR区域左上角Y or 0,
                     步骤.OCR区域右下角X or 0, 步骤.OCR区域右下角Y or 0),
            目标文本=步骤.OCR目标文本 or "",
            识别语言=步骤.OCR识别语言 or "中文简体+英文",
            超时时间=步骤.OCR超时时间 or 10,
            轮询间隔=步骤.OCR轮询间隔 or 500,
            超时处理=步骤.OCR超时处理 or "跳过继续",
        )
        try:
            return self.OCR条件判断器.轮询检测(条件, 条件.超时时间, 条件.轮询间隔)
        except Exception:
            return 条件.超时处理 == "跳过继续"
