...
class 步骤详情组件类(QWidget):
    ...
    def _创建所有字段(self) -> None:
        ...
        # 调用脚本相关
        self._添加字段组("调用脚本参数")
        self._添加字段("引用脚本标识", "引用脚本ID")

        # 分支信息
        self._添加字段组("分支信息")
        self._添加字段("父步骤标识", "父步骤标识")
        self._添加字段("分支类型", "分支类型")
    ...
    def 显示步骤详情(self, 步骤: 操作步骤数据) -> None:
        ...
        self._设置字段值("父步骤标识", 步骤.父步骤标识)
        self._设置字段值("分支类型", 步骤.分支类型)

        self._设置字段值("引用脚本标识", 步骤.引用脚本标识)

        # 根据操作类型控制字段可见性
        self._更新字段可见性(步骤.操作类型, 步骤)
    ...
    def _更新字段可见性(self, 操作类型: str, 步骤: 操作步骤数据 | None = None) -> None:
        """根据操作类型显示/隐藏相关字段"""
        try:
            类型枚举 = 操作类型枚举(操作类型)
        ...
        是延时 = (类型枚举 == 操作类型枚举.延时)
        是OCR = (类型枚举 == 操作类型枚举.OCR条件判断)
        是调用脚本 = (类型枚举 == 操作类型枚举.调用脚本)
        是子步骤 = (步骤 is not None and 步骤.父步骤标识 is not None)
        ...
        self._设置字段组可见(调用脚本字段, 是调用脚本)
        self._设置字段组可见(分支字段, 是子步骤)
