...
def 连接信号槽(主窗口, 执行引擎, 录制控制器, 脚本管理服务, 步骤管理服务,
    ...
    脚本列表.设置定时任务信号.connect(lambda 标识: _设置定时任务(标识, 定时调度器, 脚本管理服务, 脚本列表))
    ...
    定时调度器.定时激活信号.connect(
        lambda 任务: (脚本列表.设置定时任务状态(任务.关联脚本标识, 任务.任务标识, 任务),
                     执行控制.设置回放禁用(True)))
...
def _设置定时任务(脚本标识, 定时调度器, 脚本管理服务, 脚本列表):
    """从右键菜单设置定时任务"""
    from src.表现层.定时任务管理组件 import 定时任务对话框
    对话框 = 定时任务对话框(脚本管理服务, 脚本列表, 默认脚本标识=脚本标识)
