...
def 初始化数据访问层():
    ...
    return 数据库管理器, 脚本DAO, 步骤DAO, 热键DAO, 配置DAO, JSON序列化器


def 初始化业务逻辑层(脚本DAO, 步骤DAO, 热键DAO, 配置DAO, JSON序列化器):
    """初始化业务逻辑层，返回各组件实例"""
    鼠标执行器 = 鼠标操作执行器类()
    按键执行器 = 按键操作执行器类()
    ...
    脚本管理服务 = 脚本管理服务类(脚本DAO, 步骤DAO, JSON序列化器)
    步骤管理服务 = 步骤管理服务类(步骤DAO, 脚本管理服务)

    回放控制器 = 回放控制器类(鼠标执行器, 按键执行器, OCR服务, OCR条件判断器, 步骤管理服务)
    执行引擎 = 执行引擎类(回放控制器, 脚本管理服务, 步骤管理服务)
    录制控制器 = 录制控制器类(脚本管理服务, 步骤管理服务)

    定时调度器 = 定时调度器类(脚本DAO, 执行引擎)
    热键管理器 = 热键管理器类(热键DAO)
...
def 连接信号槽(主窗口, 执行引擎, 录制控制器, 脚本管理服务, 步骤管理服务,
    ...
    脚本列表.设置定时任务信号.connect(lambda 标识: _设置定时任务(标识, 定时调度器, 脚本列表))
    脚本列表.停止定时任务信号.connect(lambda: _停止定时任务(定时调度器, 脚本列表))
    ...
    执行控制.停止回放信号.connect(执行引擎.停止执行)
    执行控制.紧急停止信号.connect(执行引擎.停止执行)
    执行控制.录制信号.connect(lambda: _切换录制(录制控制器, 执行控制, 脚本列表))

    执行引擎.状态变更信号.connect(lambda 状态, 进度: _更新状态(主窗口, 执行控制, 悬浮窗, 执行引擎, 状态, 进度, 定时调度器))
    ...
    热键管理器.热键触发信号.connect(lambda 功能: _处理热键(功能, 执行引擎, 录制控制器, 执行控制, 脚本列表, 定时调度器))
    ...
    # 连接定时调度器信号，更新脚本列表定时状态和回放控件禁用状态
    定时调度器.定时激活信号.connect(
        lambda 脚本标识: (脚本列表.设置定时任务状态(脚本标识, 脚本管理服务.脚本DAO.查询ById(脚本标识)),
                         执行控制.设置回放禁用(True)))
    定时调度器.定时停止信号.connect(
        lambda: (脚本列表.清除定时状态(),
                执行控制.设置回放禁用(False)))

    # 连接热键设置和悬浮窗设置按钮信号
    执行控制.热键设置信号.connect(lambda: _显示热键设置(主窗口))
    执行控制.悬浮窗设置信号.connect(lambda: _显示悬浮窗设置(悬浮窗, 配置DAO))
...
def _导入脚本(脚本管理服务, 脚本列表):
    ...
    if 路径:
        try:
            新标识 = 脚本管理服务.从JSON导入(路径)
            脚本列表.选中脚本(新标识)
        except Exception as 异常:
            QMessageBox.warning(None, "导入失败", str(异常))


def _设置定时任务(脚本标识, 定时调度器, 脚本列表):
    """从右键菜单设置定时任务"""
    from src.表现层.定时任务管理组件 import 定时任务对话框
    对话框 = 定时任务对话框(脚本列表)
    if 对话框.exec() == 对话框.DialogCode.Accepted:
        触发类型, 触发时间, 循环间隔, 每日时间, 启用 = 对话框.获取定时配置()
        if 定时调度器:
            定时调度器.创建任务(脚本标识, 触发类型, 触发时间, 循环间隔, 每日时间, 启用)
            # 定时激活信号会自动触发脚本列表和回放控件的更新


def _停止定时任务(定时调度器, 脚本列表):
    """从右键菜单停止定时任务"""
    定时调度器.停止活动任务()
    # 定时停止信号会自动触发脚本列表和回放控件的更新


def _获取上一个脚本标识(脚本管理服务, 脚本列表, 标识) -> int:
    """获取删除标识对应脚本的上一个脚本标识"""
    脚本列表_ = 脚本管理服务.查询所有脚本()
    标识列表 = [s.脚本标识 for s in 脚本列表_]
    索引 = 标识列表.index(标识) if 标识 in 标识列表 else 0
    if 索引 > 0:
        return 标识列表[索引 - 1]
    if len(标识列表) > 1:
        return 标识列表[1]
...
def _删除脚本(脚本管理服务, 脚本列表, 标识):
    上一个标识 = _获取上一个脚本标识(脚本管理服务, 脚本列表, 标识)
    脚本管理服务.删除脚本(标识)
    脚本列表.选中脚本(上一个标识)
...
def _执行回放(执行引擎, 脚本列表, 速度, 次数):
    ...
    if 脚本标识:
        if 执行引擎.定时任务激活:
            ...
            QMessageBox.warning(None, "无法回放", "定时任务正在运行，请先停止定时任务后再回放脚本")
            return
...
def _更新状态(主窗口, 执行控制, 悬浮窗, 执行引擎, 状态, 进度, 定时调度器=None):
    try:
        运行状态 = 运行状态枚举(状态)
        ...
        回放中 = 运行状态 == 运行状态枚举.回放中
        执行控制.设置回放状态(回放中)
        # 定时任务存在时，回放控件保持禁用（回放完成后也不重新启用）
        if not 回放中 and 定时调度器:
            执行控制.设置回放禁用(定时调度器.是否有活动任务())
        # 更新脚本列表运行状态
        脚本列表 = 主窗口.脚本列表组件
        脚本标识 = 执行引擎.当前执行脚本标识 if 执行引擎 else 0
        脚本列表.更新脚本运行状态(脚本标识, 运行状态.value)
    except Exception:
        pass


def _处理热键(功能, 执行引擎, 录制控制器, 执行控制, 脚本列表, 定时调度器=None):
    日志 = 获取日志管理器("热键处理")
    日志.info(f"处理热键: {功能}")
    if 功能 in ("启动录制", "停止录制"):
        ...
        if 执行引擎.当前状态 == 运行状态枚举.回放中:
            执行引擎.停止执行()
        elif 定时调度器 and 定时调度器.是否有活动任务():
            日志.warning("定时任务正在运行，无法手动回放")
        else:
            _执行回放(执行引擎, 脚本列表, 1.0, 1)
    elif 功能 == "紧急停止":
        执行引擎.停止执行()
    else:
        日志.warning(f"未识别的热键功能: {功能}")
...
def _显示热键设置(主窗口):
    ...
    对话框.配置保存信号.connect(
        lambda: 主窗口.悬浮窗.更新热键按钮文字(主窗口.热键管理器.获取当前配置())
    )
    对话框.exec()
    ...
    try:
        ...
        (鼠标执行器, 按键执行器, OCR服务, OCR条件判断器,
         ...
         录制控制器, 定时调度器, 热键管理器, 配置DAO_业务层) = 初始化业务逻辑层(
            脚本DAO, 步骤DAO, 热键DAO, 配置DAO, JSON序列化器)
        ...
        主窗口.脚本列表组件.刷新列表()
        主窗口.脚本列表组件.选中脚本(0)
        # 恢复定时任务的UI状态（如果应用重启前有活动的定时任务）
        if 定时调度器.当前活动脚本标识:
            主窗口.脚本列表组件.设置定时任务状态(
                定时调度器.当前活动脚本标识,
                脚本管理服务.脚本DAO.查询ById(定时调度器.当前活动脚本标识))
            主窗口.执行控制组件.设置回放禁用(True)
        主窗口.show()
        系统托盘.show()
